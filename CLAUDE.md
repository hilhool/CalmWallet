...
- Google Gemini API — пакет `@google/generative-ai`, модель `gemini-2.0-flash`
...
- Функция `analyzeTransaction({ amount, category, emotional_state, triggered_by, note, history })`
- `history` — последние 5 трат пользователя, чтобы коуч видел паттерн
- System prompt: CBT/ACT коуч, тёплый тон, 2–4 предложения, без диагнозов
- Таймаут 15 с, один retry при 429/503
- Ошибка Gemini — не фатальна: транзакция сохраняется с `ai_response = null`
